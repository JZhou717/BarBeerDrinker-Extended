...
export class DrinkerDetailsComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private drinkerService: DrinkersService) {
    ...
    this.drinkerService.getDrinkerSpending(this.drinkerName).subscribe(
      data => {
        console.log(data);
        const days = [];
        const spent = [];
        data.forEach(day => {
          days.push(day.day);
          spent.push(day.totalSpending);
        })
        this.renderChart2(days,spent);
  ...
  renderChart2(days: string[], spent: number[]){
